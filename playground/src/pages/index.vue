...
const streamChunkSize = useLocalStorage<number>(
  "vmr-settings-stream-chunk-size",
  1
);
const normalizedChunkSize = computed(() =>
  Math.max(1, Math.floor(streamChunkSize.value) || 1)
);
...
watchEffect(() => {
  ...
  if (streamDelay.value !== boundedDelay) streamDelay.value = boundedDelay;
});
  ...
  preTransformTokens: (tokens: any[]) => {
    ...
    return tokens.map((token) => {
      ...
      if (
        ...
        token.content.startsWith("<thinking>")
      ) {
...
const selectedTheme = useLocalStorage<string>(
  "vmr-settings-selected-theme",
  "vitesse-dark"
);
...
function performAutoScrollIfNeeded(force = false) {
  ...
  if (timeSinceLastScroll > 16) doImmediateScroll();

  if (scrollCheckTimeoutId !== null) clearTimeout(scrollCheckTimeoutId);
...
function setupContentResizeObserver() {
  ...
  contentResizeObserver = new ResizeObserver(() => {
    ...
    if (currentHeight > lastContentHeight) forceScrollWithFrames(3);
...
<template>
  <div
    class="flex items-center justify-center p-4 app-container h-full bg-gray-50 dark:bg-gray-900"
  >
    <!-- 设置按钮和面板 -->
        ...
        >
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-3">
              ...
              />
              <span
                class="text-xs font-medium text-gray-600 dark:text-gray-400 w-12 text-right"
              >
                {{ streamDelay }}ms
              </span>
            </div>
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :style="{
                ...
                transition:
                  'background-color 0.35s ease-out, box-shadow 0.2s ease, transform 0.1s ease',
              }"
            ...
            >
              ...
              <div
                ...
                :style="{
                  ...
                  transition:
                    'left 0.35s cubic-bezier(0.25, 0.46, 0.45, 0.94), transform 0.2s ease-out, box-shadow 0.2s ease',
                }"
              >
                <!-- 图标根据状态显示 -->
                <Transition
    ...
    <div
      class="chatbot-container max-w-5xl w-full bg-white dark:bg-gray-800 rounded-2xl shadow-2xl dark:shadow-gray-900/50 flex flex-col overflow-hidden border border-gray-200 dark:border-gray-700"
    >
      <!-- Header -->
      <div
        class="chatbot-header px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-gray-800 dark:to-gray-800"
      >
        <div class="flex items-center justify-between gap-3">
          <div class="flex items-center gap-3">
            <div
              class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-lg"
            >
      ...
      <main
        ...
        class="chatbot-messages flex-1 overflow-y-auto mr-[1px] mb-4"
      ...
      >
        ...
        <div
          ref="bottomSentinel"
          aria-hidden="true"
          class="w-full h-1 pointer-events-none"
        />
      </main>
    </div>
  </div>
