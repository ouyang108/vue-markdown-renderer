...
const md = getMarkdown("hi", {
  markdownItOptions: {
    highlight: (str: string, lang: string) => {
      ...
      if (!(_lang in languageMap)) lang = "plaintext";
...
md.renderer.rules.math_inline = (tokens: any[], index: number) => {
  ...
  if (rendered) return `<span class="math-inline">${rendered}</span>`;
...
md.renderer.rules.math_block = (tokens: any[], index: number) => {
  ...
  if (rendered) return `<div class="math-block my-4">${rendered}</div>`;
  ...
  >
    ...
    >
      ...
      <main
        ...
        class="chatbot-messages prose prose-sm max-w-full flex-1 overflow-y-auto mr-[1px] mb-4"
